...
public class ObjectUtil {
	...
	{
		return Users.builder().id(dto.getId())
	...
	public static UsersDto entityToDto(Users dto){
		return UsersDto.builder().username(dto.getUsername()).id(dto.getId()).email(dto.getEmail()).password(dto.getPassword()).createdAt(dto.getCreatedAt())
				.build();
	...
	public static PetsDto petsEntityToDto(Pets dto)
	{
		return PetsDto.builder().petId(dto.getPetId()).species(dto.getSpecies()).
				...
				location(dto.getLocation()).adoptionFee(dto.getAdoptionFee()).build();
	}
